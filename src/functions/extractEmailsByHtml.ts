import { CheerioAPI, load } from "cheerio";

const EMAIL_REGEX =
  /([a-z0-9][-a-z0-9_\+\.]*[a-z0-9])@([a-z0-9][-a-z0-9\.]*[a-z0-9]\.(arpa|root|aero|biz|cat|com|coop|edu|gov|info|int|jobs|mil|mobi|museum|name|net|org|pro|tel|travel|ac|ad|ae|af|ag|ai|al|am|an|ao|aq|ar|as|at|au|aw|ax|az|ba|bb|bd|be|bf|bg|bh|bi|bj|bm|bn|bo|br|bs|bt|bv|bw|by|bz|ca|cc|cd|cf|cg|ch|ci|ck|cl|cm|cn|co|cr|cu|cv|cx|cy|cz|de|dj|dk|dm|do|dz|ec|ee|eg|er|es|et|eu|fi|fj|fk|fm|fo|fr|ga|gb|gd|ge|gf|gg|gh|gi|gl|gm|gn|gp|gq|gr|gs|gt|gu|gw|gy|hk|hm|hn|hr|ht|hu|id|ie|il|im|in|io|iq|ir|is|it|je|jm|jo|jp|ke|kg|kh|ki|km|kn|kr|kw|ky|kz|la|lb|lc|li|lk|lr|ls|lt|lu|lv|ly|ma|mc|md|mg|mh|mk|ml|mm|mn|mo|mp|mq|mr|ms|mt|mu|mv|mw|mx|my|mz|na|nc|ne|nf|ng|ni|nl|no|np|nr|nu|nz|om|pa|pe|pf|pg|ph|pk|pl|pm|pn|pr|ps|pt|pw|py|qa|re|ro|ru|rw|sa|sb|sc|sd|se|sg|sh|si|sj|sk|sl|sm|sn|so|sr|st|su|sv|sy|sz|tc|td|tf|tg|th|tj|tk|tl|tm|tn|to|tp|tr|tt|tv|tw|tz|ua|ug|uk|um|us|uy|uz|va|vc|ve|vg|vi|vn|vu|wf|ws|ye|yt|yu|za|zm|zw)|([0-9]{1,3}\.{3}[0-9]{1,3}))/i;

const BLACKLISTED_EMAILS_REGEX = /sentry|^name|^your|yext\.com$|@2x/i;

const validateEmail = (email: string | null) => {
  if (email === null) return null;
  if (!EMAIL_REGEX.test(email)) return null;

  const matched = email.match(BLACKLISTED_EMAILS_REGEX);
  if (matched !== null && matched.length > 0) return null;

  // remove quotes if they exist
  email = email.replace(/"(.*)"/, "$1");
  return email.toLowerCase();
};

const extractEmailByAttrs = ($: CheerioAPI) => {
  const attrs = ["mailto:"];

  for (const attr of attrs) {
    const elements = $(`[href^='${attr}']`);

    for (const element of elements) {
      let href = $(element).attr("href");
      if (href === undefined) return null;

      return href
        .replace(attr, "")
        .replace(/${attr}/g, "")
        .replace(/\?.*/g, "");
    }
  }

  return null;
};

export default (html: string) => {
  const $ = load(html);
  let email = extractEmailByAttrs($);

  if (email === null) {
    let execArr: RegExpExecArray | null = null;

    while ((execArr = EMAIL_REGEX.exec(html)) !== null) {
      return validateEmail(execArr[0]);
    }
  }

  return validateEmail(email);
};
